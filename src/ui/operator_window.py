...
class OperatorWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("CyberFranco - Operator Console")
        self.resize(900, 650)

        self.repository = ParticipantRepository(
            Path("data/partecipanti_example.xlsx")
        )

        self.participants = self.repository.load()
        self.matcher = NameMatcher(self.participants)
    ...
    def _build_ui(self):
        ...
        self.status_label = QLabel(
            f"Partecipanti caricati: {len(self.participants)}"
        )
    ...
    def _confirm_item(self, item):
        self.selected_participant = item.data(
            Qt.UserRole
        )
